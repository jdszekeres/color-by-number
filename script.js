...
async function start() {
    game_req = await fetch("images.json")
...
function draw_puzzle(data) {
    for (i = 0; i < data.drawing.length; i++) {
        ...
        for (j = 0; j < data.drawing[i].length; j++) {
            ...
            color_tile.addEventListener("mouseover", (function (i, j, c, d) {
                return function () {
                    fill_tile(i, j, c, d);
                };
            })(i, j, color_tile, data))
...
function settings_modal() {
    ...
    wallpapers.forEach((wallpaper) => {
        ...
        button.onclick = () => {
            ...
            Array.from(document.getElementsByClassName('wallpaper')).forEach((paper) => {
                if (paper.childNodes[0].src !== wallpaper) { paper.style.backgroundColor = "transparent"; }
    ...
    setTimeout(() => Array.from(document.getElementsByClassName('wallpaper')).forEach((paper) => {
        if (paper.childNodes[0].src !== temp_paper) { paper.style.backgroundColor = "transparent"; }
        else { paper.style.backgroundColor = "yellow" }
    }), 100)
